...
if (isset($_POST["action"])) {
	...
	if ($_POST["action"] == "load_friend_request_list") {
		...
		foreach ($result as $row) {
			...
			<li>
				...
				<button type="button" name="accept_friend_request_button" class="btn btn-primary btn-xs pull-right accept_friend_request_button" data-request_id="' . $row["request_id"] . '" id="accept_friend_request_button_' . $row["request_id"] . '"><i class="fa fa-plus" aria-hidden="true"></i> Accepter</button>
			</li>
			';
		}
		echo $output;
	...
	if ($_POST["action"] == 'like_post') {
		...
		if($output == 0){
			sleep(1);
			$query = "
			INSERT INTO aimer (posts_id, user_id,nb_like)
 			VALUES ('" . $_POST['like_id'] . "', '" . $_SESSION["user_id"] . "', '1')
			";
			$connect->query($query);
		}
	...
	if ($_POST["action"] == 'load_friends') {
		...
		if (!empty($_POST["query"])) {
			...
			foreach ($search_array as $search) {
				if (trim($search) != '') {
					$condition .= "Utilisateur.pseudo LIKE '%" . $search . "%' OR ";
				}
			}
		...
		$query = "
		SELECT Utilisateur.pseudo, friend_request.request_from_id, friend_request.request_to_id FROM Utilisateur 
		INNER JOIN friend_request 
		ON friend_request.request_from_id = Utilisateur.id_user
		OR friend_request.request_to_id = Utilisateur.id_user
		WHERE (friend_request.request_from_id = '" . $_SESSION["user_id"] . "' OR friend_request.request_to_id = '" . $_SESSION["user_id"] . "') 
		AND Utilisateur.id_user != '" . $_SESSION["user_id"] . "'
		...
		if ($statement->rowCount() > 0) {
			...
			foreach ($statement->fetchAll() as $row) {
				...
				<div class="col-md-4" style="margin-bottom:12px;">
					...
					<div align="center"><b><a href="profilami.php?id=' . $temp_user_id . '" style="font-size:12px;">' . Get_user_name($connect, $temp_user_id) . '</a></b></div>
				</div>
				';
			...
			$html = '<h4 align="center">Pas damis trouve</h4>';
		}
		echo $html;
	}
	if ($_POST["action"] == 'remove_friend') {
		...
		WHERE (request_from_id='" . $_SESSION['user_id'] . "' AND request_to_id='" . $_POST['id_ami'] . "')
		 OR (request_to_id='" . $_SESSION['user_id'] . "' AND request_from_id='" . $_POST['id_ami'] . "')
		";
